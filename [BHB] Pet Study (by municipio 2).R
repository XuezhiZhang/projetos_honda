...
require(ggplot2)
require(data.table)
require(dvmisc) #for function quant_groups (break in percentiles (as many as u desire))

load("bhb_fecha_dez_18.RData")

out = bhb.fecha.dez.18 %>% dplyr::filter(qtd_parc_restantes > 0 & qtd_dias_em_atraso <= 360 & situacao_contrato %in% c("CCA","CCJ","CEA","CED")) %>% dplyr::group_by(nome_est_loja, segmento) %>% dplyr::summarise(outstanding = sum(qtd_itens, na.rm = TRUE))
del = bhb.fecha.dez.18 %>% dplyr::filter(qtd_parc_restantes > 0 & qtd_dias_em_atraso >= 31 & qtd_dias_em_atraso <= 360 & situacao_contrato %in% c("CCA","CCJ","CEA","CED")) %>% dplyr::group_by(nome_est_loja, segmento) %>% dplyr::summarise(delinquents = sum(qtd_itens, na.rm = TRUE))

all = full_join(out, del)
all = all %>% mutate_if(is.numeric, funs(replace(., is.na(.), 0)))

all = all %>% mutate(index = delinquents/outstanding)
all$median.deliq = median(all$delinquents)

all$id = seq.int(nrow(all))
all$dec = quant_groups(all$delinquents, groups = 15)

set.seed(363)
id1 = all %>% group_by(dec, segmento) %>% sample_frac(0.5)

id1 = c(id1$id)

all$assessoria = ifelse(all$id %in% id1, "PASCHOALOTTO", "FLEX CONTACT")

index_by_sample = valid %>% group_by(assessoria, segmento) %>% summarise(outstanding = sum(outstanding, na.rm = TRUE),
...
fwrite(index_by_sample, file = "index_by_sample.csv", sep = ";", dec = ",")
...
valid = fread("index_by_state.txt", h = T, dec = ",")
...
count = all %>% group_by(segmento, nome_est_loja, assessoria) %>% summarise(delinquents = sum(delinquents))
...
ggplot(data=all, aes(x=nome_est_loja, y=delinquents, fill=assessoria)) +
  geom_bar(stat="identity", position = "fill") +
  facet_wrap(~segmento) + coord_flip() +
  labs(title = "Digital Rule | Balanced distribution of delinquents by state and segment",
