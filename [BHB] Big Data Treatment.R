...
modelling <- function(mod_segment, model, segment){
  ...
  segment <<- segment
      ...
      dev.off()
          ...
          perc = table(dbs$train_db_woe$target)[2]/(table(dbs$train_db_woe$target)[1]+table(dbs$train_db_woe$target)[2])
          lambda = ifelse(perc < 0.75, 
                          cv.lasso$lambda.1se, 
          ...
          bwplot(results, ncol = 3)
              ...
              probabilities = c(predict(model.down$finalModel, newx = X.test, 
                                      type="response", s = model.down$bestTune$lambda))
              ...
              evaluation <- scorecard::perf_eva(observed.classes.num, predicted.classes.num, show_plot = TRUE)
              df <- data.frame(KS = evaluation$KS, AUC = evaluation$AUC, Gini = evaluation$Gini)
              df = melt(df); names(df) <- c("Statistic", "Value")
              ...
              "Step 4: Predictions and model performance measures done.\n\n" %>% cat()
                  ...
                  conf_db = as.data.frame(as.matrix(conf_matrix, what = "overall")); conf_db = rownames_to_column(conf_db, "Statistic")
                  conf_db2 = as.data.frame(as.matrix(conf_matrix, what = "classes")); conf_db2 = rownames_to_column(conf_db2, "Statistic")
                  conf = bind_rows(conf_db, conf_db2); colnames(conf) <- c("Statistic", "Value"); conf$Value = round(conf$Value, 4)
                  perf_stats = bind_rows(conf, df)
                  
                  "Starting step 5: Writing all needed files in their paths.\n\n" %>% cat()
                  writing(glm.model, selected_var, probs_by_contract, imp, desired_model, segment, perf_stats)
